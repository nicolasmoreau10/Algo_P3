...
if not ((player1 == "Sélectionnez un joueur") or (player2 == "Sélectionnez un joueur")) and meme_joueur == False :
    ...
    col1.write(f"Age (dernier match) : {floor(info[1])} ans")
    arrondiInf = floor(12.3)
    col1.write(f"Nationalité : {info[2]}")

    col2.markdown("<h1 style='text-align: center; color : #ac211e'>VS</h1>", unsafe_allow_html=True)
    ...
    col3.write(f"Age (dernier match) : {floor(info[1])} ans")   
    ...
    if (historique[-1] == 0) and (historique[-2] == 0) :
        st.markdown("<h3 style='text-align: center; color:red'>Pas de passif d'affrontement entre ces 2 joueurs !</h3>", unsafe_allow_html=True)
    else :
        ...
        st.markdown("<h2 style='text-align: center;'> HISTORIQUE TAUX VICTOIRE VS</h2>", unsafe_allow_html=True)
        st.markdown(f"<h2 style='text-align: center; color : #ac211e'>{nb_duels} affrontements entre {premier_duel} et {dernier_duel}.</h2>", unsafe_allow_html=True)
        fig = go.Figure(
        go.Pie(
    ...
    st.markdown("<h2 style='text-align: center;'> VICTOIRES / DEFAITES </h2>", unsafe_allow_html=True)
...
#     )
    ...
    st.markdown("<h2 style='text-align: center;'> TAUX VICTOIRE PAR SURFACE </h2>", unsafe_allow_html=True)
#     col1, col2 = st.columns(2)
...
#         col2.metric(label=f"Winrate sur {surface}", value= f"{txj2}%", delta=f"{deltaTx2}%")
    ...
    surfaces = ['Clay', 'Grass', 'Hard']
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=taux_victoire_player1,theta=surfaces,fill='toself', name=f'{player1}',))
    fig.add_trace(go.Scatterpolar(r=taux_victoire_player2,theta=surfaces,fill='toself',name=F'{player2}', line=dict(color='rgba(255, 0, 0, 0.5)')))
    fig.update_layout(
    font=dict(size=17),
    ...
    legend=dict(
        ...
        xanchor="center",
        yanchor="top",
        font=dict(size=17# Taille de la police de la légende
    )),
    ...
    st.write("")
    st.markdown("<h2 style='text-align: center;'> STYLES DE JEU </h2>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    
    if df_joueur['joueur'].isin([player1]).any() :
        ...
        fig1.update_layout(
        title=dict(text = player1, x=0.5, y=0.9, font=dict(size=20), xanchor='center', yanchor='top'),
        margin=dict(t=50, b=0),)
        col1.plotly_chart(fig1, use_container_width=True)
    else :
        col1.markdown(f"<h5 style='text-align: center; color : red; padding-top: 200px'>Pas de données sur {player1}</h5>", unsafe_allow_html=True)
        
    if df_joueur['joueur'].isin([player2]).any() :
        ...
        fig2.update_layout(
        title=dict(text = player2, x=0.5, y=0.9, font=dict(size=20), xanchor='center', yanchor='top'),
        margin=dict(t=50, b=0),)
        col2.plotly_chart(fig2, use_container_width=True)
    else :
        col2.markdown(f"<h5 style='text-align: center; color : red; padding-top: 200px'>Pas de données sur {player2}</h5>", unsafe_allow_html=True)
...
#     st.plotly_chart(fig)
    ...
    st.markdown("<h2 style='text-align: center; color : #ac211e'>AND NOW, LADIES AND GENTLEMEN...</h2>", unsafe_allow_html=True)
    ...
    if choix_surfaces != 'Choisissez une surface' :
        ...
        if bouton :
            ...
            try :
                ...
                print("Taux vicJ1 & VJ1: ")
                print(taux_vic_J1_matchs_h2h, vJ1)
                print("Taux vicJ2 & VJ2: ")
                print(taux_vic_J2_matchs_h2h, vJ2)
                
                #Nous créons un seuil minimal de match pour juger si la tendance est significative et éviter de pondérer
                # gravement avec un coefficient de 0 les moyennes des joueurs. 
                seuil= len(duels_surface)
                if seuil <4 and (vJ1==0 or vJ2==0):
                    taux_vic_J1_matchs_h2h = 1
                    taux_vic_J2_matchs_h2h = 1  
                    st.write("")
                    st.markdown("<h5 style='text-align: center; color:red'>NB : Nombre de matchs peu représentatif pour cette surface entre les joueurs</h5>", unsafe_allow_html=True)
            except :
                taux_vic_J1_matchs_h2h = 1
                taux_vic_J2_matchs_h2h = 1
                
#             st.write(f"taux de victoire de {player1} contre {player2}", taux_vic_J1_matchs_h2h)
#             st.write(f"taux de victoire de {player2} contre {player1}", taux_vic_J2_matchs_h2h)
...
#             st.write("FEATURES AVANT APPLICATION DU H2H")
#             st.write(f"{player1}",player1_features)
#             st.write(f"{player2}",player2_features)
...
#             st.write("FEATURES APRES APPLICATION DU H2H")
#             st.write(f"{player1}",player1_features)
#             st.write(f"{player2}",player2_features)
            ...
            proba_winner = model.predict_proba(concatenated_array.reshape(1, -1))
            

            # Affichage du résultat
                                   
            if winner == 'player_1' :
                if proba_winner[0,0] >= 0.9 :
                    col2.success(f"{player1} aurait plus de 90% de chance de gagner")
                else :
                    col2.success(f"Le gagnant serait {player1} avec une probabilité de {proba_winner[0,0]*100:.1f} %")
                try:
                    imagej1 = Image.open(path + "\\photos\\" + player1.split(' ')[1].lower()+".png")
                ...
                if proba_winner[0,1] >= 0.9 :
                    col2.success(f"{player2} aurait plus de 90% de chance de gagner")
                else :
                    col2.success(f"Le gagnant serait {player2} avec une probabilité de {proba_winner[0,1]*100:.1f} %")
                try:
                    imagej2 = Image.open(path + "\\photos\\" + player2.split(' ')[1].lower()+".png")
                    overlay = Image.open(path + "\\photos\\" + "coupe.png")
